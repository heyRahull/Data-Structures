...
unordered_map<string,bool>mp;  //creating a map
...
bool solve(string a,string b)
{
    ...
    string key=a;        //creating keys for the map
    key.push_back(' ');
    key.append(b);

    if(mp.find(key)!=mp.end()) //searching in the map
            return mp[key];
    ...
    return mp[key]=flag;//storing in the map
}
